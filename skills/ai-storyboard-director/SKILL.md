...
内部维护版本：**5.4.4**。版本号只用于项目审计、完整性校验和回退，不是用户调用参数。用户始终只调用 `ai-storyboard-director`，不得要求其选择版本、实验线或另一个镜头设计Skill。
...
以上文件全部属于5.4.4自身。任一文件缺失、路径越出当前版本根目录或指向旧版本时，本版本完整性检查失败，必须停止部署。
...
9. 场景值得鲜明设计时，内部比较至少两种结构真正不同的摄影方案；差异必须来自观看任务、调度、视点、光学、路径、信息顺序或剪辑结构，不能只是把一个运镜名换成另一个。简单镜头若最有力量可以使用，但不得因熟悉或稳妥而自动退回普通覆盖。
...
15. 把分镜写入严格六大模块；普通任务默认同轮交付，只有用户明确要求设计稿时才等待其确认。数字10只作为信息内核，不拆成十个外显章节。
