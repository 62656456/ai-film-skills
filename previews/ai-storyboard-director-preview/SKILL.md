---
name: ai-storyboard-director-preview
description: Explicit-only 5.6.0 preview for testing saved shot-design decisions, scene-focused recovery, framing and axis guidance, and deterministic continuity checks. Use only when the user explicitly requests ai-storyboard-director-preview or this preview test. Keep the daily ai-storyboard-director installation separate; do not activate for ordinary storyboard requests.
---

# AI Storyboard Director Preview

内部能力版本：**5.6.0**；公开测试包：**5.6.0-rc.1**。本包仅供用户明确调用 `ai-storyboard-director-preview` 时测试，不能接管普通分镜请求。日常 `ai-storyboard-director` 与本包分开安装，5.4.4继续作为当前日常基线。不得把本候选的安装、程序通过或自审自动当作采用新版；采用决定须等待用户实际制作检验。
...
16. 仅在用户明确测试本候选时执行；交付外单独标记Preview，不能把测试标识塞进可复制视频提示词，也不能改动日常版本。
