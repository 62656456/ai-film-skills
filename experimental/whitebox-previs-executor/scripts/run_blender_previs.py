#!/usr/bin/env python3
"""Locate Blender and execute the prompt-previs adapter without showing its UI."""
...
def main() -> int:
    ...
    if result["status"] != "pass":
        ...
        return 2
    if data.get("action_program") is not None or data.get("meta", {}).get("execution_backend") == "rigged_joint_solver_v1":
        print("Fight action contracts require run_blender_rigged_fight.py; the ordinary backend cannot substitute root motion.", file=sys.stderr)
        return 2
    script = Path(__file__).resolve().with_name("blender_previs_adapter.py")
    search_root = Path.cwd().resolve()
    try:
        blender = find_blender(args.blender, search_root)
